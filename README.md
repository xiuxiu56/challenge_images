...
去重后真实不均衡是 550 倍（Car 9894 / Tractor 18）。不加权时模型把 Tractor
全部预测错只损失总体准确率的万分之几，长尾类几乎没有梯度信号。

- 单标签：加权交叉熵，权重按样本数倒数并以中位数归一化
- 多标签：BCE 的 `pos_weight`，定义为负/正样本比

两者都用 sqrt 平滑并截断到 20×——原始 550 倍差距会让单个样本主导整批梯度。
默认启用，可用 `balance_classes=False` 关闭。

### 域相关数据增强

`auto_augment=augmix` 模拟的是自然照片的光照与几何变化，但本任务的域差异
是**压缩与分辨率损失**：图块从 JPEG 大图切出，浏览器按 CSS 尺寸缩放后显示。

因此额外叠加四项 PIL 级变换：JPEG 重编码（q=45~95）、降采样至 60% 再还原、
轻微高斯模糊、亮度对比度抖动。只对训练集生效，验证集保持确定性。

> 注意：Ultralytics 在 `auto_augment` 开启时会禁用自带色彩抖动，
> 配置里的 `hsv_h/hsv_s/hsv_v` 在 augmix 打开时实际不生效。
...
1. **图片尺寸**——13707 张图上「尺寸 → 网格」无一例外：300×300→3×3，
   450×450→4×4。直接的像素证据，不受接口字段是否新鲜影响。
2. **已知挑战类型**——实测类型映射 100% 可靠。
3. **pmeta 检测值**——仅用于类型未知时。pmeta 观察到会残留上一轮的值。

两处已修正的缺陷：

- `grid_from_pmeta` 原读下标 [2],[3]，正确位置是 [3],[4]。在 8626 条记录上
  统计，正确下标命中率 100%，旧下标对全部 4×4 挑战返回 `None`。
- `tileselect` 原配置为 3×3，实测为 450×450 且 pmeta 写 4,4，已改为 4×4。

### 4×4 连通性约束

4×4 是一张连续照片，目标在网格上必然连通。但「只保留最大连通域」是错的——
画面里可以有两棵棕榈树、两个红绿灯。采用的规则是**孤立单格需要更强的分类
证据**（默认 0.70），且仅在同时存在更大连通块时生效，避免整图只有一个小目标
时误删。人行横道保留已调校的连续带专用逻辑。
...
缓存键按前两位十六进制分片存放（256 个桶）。菜单 `25` 提供统计、
历史平铺文件迁移、按天数或数量清理、以及清空。

缓存此前没有淘汰机制，只增不减——`online_prediction_cache` 曾累积
19530 个文件平铺在单个目录里。
